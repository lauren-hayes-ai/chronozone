...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <p className="text-gray-400 mb-8">Manage your API keys and monitor usage. (Stytch sign-in pending setup)</p>

      <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-8 text-sm text-yellow-300">
        ⚠️ Stytch integration pending — dashboard functionality will be enabled once Stytch credentials are configured.
      </div>

      {newKey && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-6 mb-8">
          <div className="text-sm text-amber-400 font-medium mb-2">New API Key Created</div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-8">
        ...
        <div className="flex gap-3">
          <input type="text" value={keyName} onChange={(e) => setKeyName(e.target.value)} placeholder="Key name" className="bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-sm flex-1 focus:outline-none focus:border-amber-500" />
          <button onClick={createKey} className="bg-amber-600 hover:bg-amber-500 px-6 py-2 rounded-lg text-sm font-medium transition">Create Key</button>
        </div>
      </div>
