...
> {
  ...
  const keys = await sql<ApiKeyRecord[]>`
    SELECT * FROM chronozone_api_keys WHERE key_hash = ${keyHash} AND revoked_at IS NULL LIMIT 1
  `;
  ...
  const [{ count }] = await sql<{ count: number }[]>`
    SELECT COUNT(*)::int as count FROM chronozone_api_usage
  ...
  await sql`UPDATE chronozone_api_keys SET last_used_at = now() WHERE id = ${key.id}`;
  return { key, remaining: key.daily_limit - count - 1 };
}

export async function logUsage(keyId: string, endpoint: string) {
  await sql`INSERT INTO chronozone_api_usage (key_id, endpoint) VALUES (${keyId}, ${endpoint})`;
}
